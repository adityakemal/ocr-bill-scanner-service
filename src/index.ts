...
app.post("/ocr/mindee/file", async ({ request }) => {
  try {
...
app.post("/ocr/mindee/base64", async ({ request }) => {
  try {
...
app.post("/ocr/mindee/url", async ({ request }) => {
  try {
    const body = await request.json();
...
app.post("/ocr/tesseract/file", async ({ request }) => {
  try {
    const form = await request.formData();
    const file = form.get("file") as File;
...
app.post("/ocr/tesseract/base64", async ({ request }) => {
  try {
    const body = await request.json();
    const { image, filename } = body as { image: string; filename?: string };
